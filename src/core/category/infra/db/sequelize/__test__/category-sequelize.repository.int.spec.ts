...
describe("CategorySequelizeRepository Integration Test", () => {
  ...
  it("should insert a new category", async () => {
    ...
    const foundCategory = await CategoryModel.findByPk(category.category_id.id);
    expect(foundCategory.toJSON()).toStrictEqual(category.toJSON());
  });

  it("should insert list category", async () => {
    const categories = [
    ...
    expect(foundCategories.map((i) => i.toJSON())).toStrictEqual(
      categories.map((i) => i.toJSON())
    );
  });
  ...
  it("should throw an error on update when category is not found", async () => {
    const category = Category.fake().aCategory().build();
    await expect(repository.update(category)).rejects.toThrow(Error);
  });

  it("should throw an error on delete when category is not found", async () => {
    const category = Category.fake().aCategory().build();
  ...
  it("should return null on findById when category is not found", async () => {
    const category = Category.fake().aCategory().build();
  ...
  describe("search method tests", () => {
    it("should only apply paginate when other params are null", async () => {
      ...
      const categories = Category.fake()
        ...
        .build();
      await repository.bulkInsert(categories);
      const spyToEntity = jest.spyOn(CategoryModel, "toEntity");
